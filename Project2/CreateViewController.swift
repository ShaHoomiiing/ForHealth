...
class CreateViewController: UIViewController, UITextFieldDelegate {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool { // delegate method
        if textField == self.textPrefix {
            ...
            self.textID.becomeFirstResponder() // Prefix 입력 후 ID로 이동
    ...
    @IBAction func buttonSave() {
        if textPrefix.text == "" {
            labelStatus.text = "Prefix를 입력하세요"
            return
        }
        if textID.text == "" {
        ...
        let urlString: String = "http://condi.swu.ac.kr/student/" + textPrefix.text! + "/login/insertUser.php"

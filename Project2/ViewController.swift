...
class ViewController: UIViewController, UITextFieldDelegate {
    ...
    func textFieldShouldReturn(_ textField: UITextField) -> Bool {
        ...
        self.loginUserid.becomeFirstResponder() // Prefix 입력 후 ID로 이동
        } else if textField == self.loginUserid {
        textField.resignFirstResponder()
        self.loginPassword.becomeFirstResponder() // ID 입력 후 비번으로 이동
        }
        textField.resignFirstResponder()
        return true
    }
    ...
    @IBAction func loginPressed() {
        ...
        let urlString: String = "http://condi.swu.ac.kr/student/" + loginPrefix.text! + "/login/loginUser.php"
